...
# curl -v http://localhost:4013/v/r4/auth/token 2>&1 | head -20

# Compare with what the launcher actually serves:
curl -s http://localhost:4013/v/r4/fhir/.well-known/smart-configuration | jq '.token_endpoint'

curl -s -X POST http://localhost:4013/v/r4/auth/token \
