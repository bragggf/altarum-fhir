...
# Replace the token endpoint test in the script with:
...
# 400 = invalid_grant (endpoint working correctly)
# 404 = endpoint doesn't exist (real problem)
if [[ "$STATUS" == "400" ]] || [[ "$STATUS" == "401" ]]; then
  pass "Token endpoint reachable and responding correctly (HTTP $STATUS)"
elif [[ "$STATUS" == "404" ]]; then
  fail "Token endpoint not found (HTTP 404) - endpoint does not exist"
else
  warn "Token endpoint returned unexpected status (HTTP $STATUS)"
fi
